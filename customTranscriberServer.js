...
const DEEPGRAM_API_KEY =
  process.env.DEEPGRAM_API_KEY || "8db50845e951f4d27e920901a1b20468d51d5407";
...
export function attachCustomTranscriberWS(server) {
  ...
  wss.on("connection", (ws) => {
    ...
    function getMaxRMSInWindow() {
      const now = Date.now();
      rmsHistory = rmsHistory.filter(
        (entry) => now - entry.time <= RMS_WINDOW_MS
      );
    ...
    ws.on("message", (msg, isBinary) => {
      if (!isBinary) {
        ...
        if (obj.type === "start") {
          console.log(
            '🚀 Received "start" — initializing Deepgram live transcription'
          );
          ...
          dgLive.on(LiveTranscriptionEvents.Transcript, (event) => {
            const alt =
              event.channel &&
              event.channel.alternatives &&
              event.channel.alternatives[0];
            const transcript = (alt && alt.transcript) || "";
            const confidence =
              alt && alt.confidence != null ? alt.confidence : null;
            ...
            if (isFinal) {
              ...
              console.log(
                `📤 FINAL ${who}: "${transcript.trim()}" | 🎯 Conf: ${confidence} | 🎙 Max RMS (last ${RMS_WINDOW_MS}ms): ${maxRMS.toFixed(
                  4
                )}`
              );
            } else {
            ...
            if (isFinal && channelIndex === 0) {
              ...
              if (fromPrimary) {
                ...
                if (maxRMS < RMS_THRESHOLD) {
                  responseText =
                    "rephrase please come closer or speak loudly your voice was not clear";
                }
                ws.send(
                  JSON.stringify({
        ...
        if (dgLive && dgLive.send) {
          ...
          console.warn(
            "⚠ Audio chunk received before Deepgram stream ready — dropped"
          );
        }
      }
    });
    ...
    ws.on("close", () => {
      console.log(
        "❌ WebSocket connection closed by client; closing Deepgram stream"
      );
      if (dgLive && dgLive.close) dgLive.close();
    });
